...
if ( trim ( $GLOBALS['stdata506'] ) === '' || ( defined( 'ST_LAZY_LOAD' ) && $GLOBALS['stdata326'] === 'yes' ) ) {
...
if ( ! function_exists( 'st_get_the_excerpt' ) ) {
	function st_theme_menu_header_add(){
		...
		$theme_name = str_replace('WING-', '', $theme_name);
...
if ( ! function_exists( 'st_has_mobile_header_content' ) ) {
	function st_has_mobile_header_content() {
		return (
			st_has_additional_mobile_menu()         // モバイル追加メニューがある
			|| trim( $GLOBALS["stdata80"] ) === ''  // スマートフォンメニューの表示が有効
			|| st_mobilelogo_on()                   // モバイル用ロゴ（又はタイトル）の使用が有効
			|| trim( $GLOBALS["stdata479"] ) !== '' // モバイルヘッダーに検索アイコンの追加が有効
		);
	}
}
...
if ( ! function_exists( 'st_has_mobile_header_content_exclude_menu' ) ) {
	function st_has_mobile_header_content_exclude_menu() {
		return (
			trim( $GLOBALS["stdata80"] ) !== ''         // スマートフォンメニューの表示が無効
			&&
			( 
				st_has_additional_mobile_menu()         // モバイル追加メニューがある
				|| trim( $GLOBALS["stdata479"] ) !== '' // モバイルヘッダーに検索アイコンの追加が有効
...
if ( ! function_exists( 'st_has_header_content' ) ) {
	function st_has_header_content() {
		...
		return (
			...
			|| ( trim( $GLOBALS["stdata428"] ) !== '' ) // ヘッダー用メニュー（横列）が有効
		);
	}
}
...
if ( ! function_exists( 'st_is_header_nav_enabled' ) ) {
	function st_is_header_nav_enabled() {
		...
		return ( wp_is_mobile() || ! $hide_slide_menu_on_pc );
	}
}
...
if ( !function_exists( 'st_pre_sc_c' ) ) {
	function st_pre_sc_c( $atts, $content = null ) {
		$atts = shortcode_atts( array(
			'myclass'       => '',
			'text'    => '',
			'fontawesome' => 'dashicons-shortcode',
		), $atts );
		...
		$myclass_class  = ( $myclass !== '' ) ? $myclass : ''; // オリジナルクラス
		$text_html   = ( $text !== '' ) ? $text : ''; //テキスト
		$fontawesome_html = ( $fontawesome !== '' ) ? '<i class="fa ' . esc_attr( $fontawesome ) . ' st-css-no" aria-hidden="true"></i> ' : '<span>[/]</span> '; //Webアイコン
...
if ( ! function_exists( 'st_input_tab_c' ) ) {
	...
	function st_input_tab_c( $atts, $content = null ) {
		...
		$fontawesome_html = ( $atts['fontawesome'] !== '' ) ? '<i class="fa ' . esc_attr( $atts['fontawesome'] ) . '" aria-hidden="true"></i>' : ''; //Webアイコン
		...
		return '<input id="' . esc_attr( $id_value ) . '" class="st-tab-label" title="' . esc_attr( $text_value ) . '" ' . $checked_attr . ' name="' . esc_attr( $name_value ) . '" type="radio" value="' . esc_attr( $value_value ) . '" ' . $event_attr . '/><label for="' . esc_attr( $id_value ) . '" style="' . $width_css . $fontweight_css . $bgcolor_css . $bordercolor_css . $color_css . '" ' . $tab_btn_class . '>' . $fontawesome_html . $text_value . '</label>';
	}
}
...
if ( ! function_exists( 'st_cardsc_func' ) ) {
	function st_cardsc_func( $arg, $content = '' ) {
		...
		$height           = ( $atts['pc_height'] !== '' ) ?  (int) $atts['pc_height'] : '';
		$type_value       = ( $atts['type'] !== '' ) ? $atts['type'] : '';

		if ( st_is_mobile() ): // モバイル閲覧時のみ
			$height_style = '';  //高さ
		else:
			$height_style = ( $height !== '' ) ? 'style="height:' . esc_attr( $height ) . 'px;overflow:hidden;"' : ''; //高さ
		endif;
...
if ( !function_exists( 'st_memo_c' ) ) {
	function st_memo_c( $atts, $content = null ){
		...
		return '<div class="clip-memobox ' . esc_attr ( $myclass_class ) . '" style="' . esc_attr( $bgcolor_css . $color_css . $border_style ) . '"><div class="clip-fonticon" style="' . esc_attr( $iconsize_css . $iconcolor_css ) . '">' . $fontawesome_html . '</div><div class="clip-memotext" style="' . esc_attr( $bordercolor_css ) . '"><p style="' . esc_attr( $color_css ) . '">'.$content.'</p></div></div>';
	}
}
...
if ( !function_exists( 'st_box_btn_list' ) ) {
	function st_box_btn_list( $atts, $content = null ) {
		...
		$fontawesome_html = ( $fontawesome !== '' ) ? '<i class="fa ' . esc_attr( $fontawesome ) . ' st-css-no" aria-hidden="true" style="' . esc_attr( $fontawesomesize_css ) . '"></i>' : ''; //Webアイコン
...
if ( !function_exists( 'st_slidebox_c' ) ) {
	function st_slidebox_c( $atts, $content = null ) {
		...
		return '<div class="st-slidebox-c" style="' . esc_attr( $margin_bottom_css . $bgcolor_css ) .'"><p class="st-btn-open" ' . $color_css . '>' . $fontawesome_html  . $text_html . '</p><div class="st-slidebox">' . $content . '</div></div>';
	}
}
...
if ( !function_exists( 'st_youtube_img' ) ) {
	function st_youtube_img( $youtube_id ) {
		...
		$youtube_link = '<div class="st-youtube"><a href="//www.youtube.com/watch?v='.$id.'" target="_blank" rel="nofollow"><i class="fa fa-youtube-play"></i><img src="//img.youtube.com/vi/'.$id.'/sddefault.jpg" alt="" width="100%" height="auto" /></a></div>';
...
if ( !function_exists( 'st_register_sidebars' ) ) {
	function st_register_sidebars() {
		...
		register_sidebar( array(
			'id' => 'sidebar-34',
			'name' => 'スライドメニュー（検索）に表示',
			'description' => 'モバイルヘッダーの検索ボタンで開くエリアに表示されます。※JSによりメニュー内の高さを取得するため挿入コンテンツによっては読み込みが遅くなる場合等がございます',
...
if ( !function_exists( 'st_eyecatch_class' ) ) {
	function st_eyecatch_class() {
		...
		} elseif ( isset($GLOBALS['stdata241']) && $GLOBALS['stdata241'] === 'photo' ) {
...
if ( !function_exists( 'st_login_ver' ) ) {
	function st_login_ver() {
		...
		$my_theme = wp_get_theme(get_template());
		echo '<!-- '. $my_theme->Name . $ex . $my_theme->Version .'-->';
	}
}
...
if (function_exists('unregister_block_pattern')) { // WordPress5.5～

	if ( !function_exists( 'st_unregister_block_pattern' ) ) {
		function st_unregister_block_pattern(){
	...
	add_action( 'init', 'st_unregister_block_pattern' );
