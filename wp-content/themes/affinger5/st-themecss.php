...
<?php if( ( st_is_mobile() && ! st_has_mobile_header_content() ) || // モバイルヘッダーのコンテンツが空
	!st_is_mobile() && ( trim($GLOBALS['stdata16']) !== '' || trim($GLOBALS['stdata80']) !== '' ) ): // PC時はスライドメニューを出力しない場合のヘッダーパディングの調整 ?>
		#headbox {
			padding: 10px!important;
		}
<?php endif; ?>

<?php if( ! st_is_mobile() && st_has_mobile_header_content_exclude_menu() ): // スマートフォンメニューの表示が無効時にそれ以外のモバイルヘッダーに出力するコンテンツがあるかどうかを返す ?>
	@media print, screen and (max-width: 599px) {
		#headbox-bg #headbox {
...
<?php if ( trim( $GLOBALS["stdata80"] ) !== '' ) : // スマートフォンメニューを表示しない ?>
...
<?php if ( trim( ($GLOBALS['stdata479']) ) === '' && isset($GLOBALS['stdata484']) && $GLOBALS['stdata484'] === 'yes' ): // 追加メニューを従来のデザインに戻す ?>
	...
	<?php if( ( trim($GLOBALS['stdata81']) !== '' || trim($GLOBALS['stdata82']) !== '') && (trim($GLOBALS['stdata83']) !== '' || trim($GLOBALS['stdata84']) !== '') ): // 追加メニュー1,2が共に存在 ?>
		/* スライドメニュー追加ボタン */
		#s-navi dt.trigger .acordion_extra_2,
		#s-navi dt.trigger .acordion_extra_1 {
			max-width: 50%;
...
<?php if ( isset($GLOBALS['stdata375']) && $GLOBALS['stdata375'] === 'yes' ): // ワイドLPの左右にシャドウ -EX ?>
	...
	/* 記事タイトルの調整 */
	.colum1.st-lp-wide.st-lp-wide-shadow #st-page .entry-title:not(.st-css-no2),
	...
	.colum1.st-lp-wide.st-lp-wide-shadow #st-page .entry-title:not(.st-css-no2)::before,
	.colum1.st-lp-wide.st-lp-wide-shadow .entry-title::before,
	.colum1.st-lp-wide.st-lp-wide-shadow #st-page .entry-title:not(.st-css-no2)::after,
...
<?php if ( $st_icon_logo_width ): ?>
	/* アイコンロゴの最大サイズ */
	#st-icon-logo img {
		max-width: <?php echo $st_icon_logo_width; ?>px;
	}
<?php endif; ?>
...
<?php if( ( $st_toc_mokuzicolor_base ) ): ?>
	...
	.post #toc_container .toc_title a {
		color: <?php echo $st_toc_mokuzicolor_base; ?>;
		border-color: <?php echo $st_toc_mokuzicolor_base; ?>;
	}
<?php endif;  ?>
...
<?php if( $st_pagetop_up || ( st_is_mobile() && is_active_sidebar( 18 ) ) ): // スマホ用フッター固定広告ウィジェット使用時 ?>
	#page-top {
		bottom: 80px;
	}
<?php endif; ?>
...
<?php if( $st_mobile_logo_center ): ?>
	...
	header p#st-mobile-logo {
		text-align: center;
		<?php if ( trim( $GLOBALS['stdata80'] ) !== '' && trim( $GLOBALS['stdata479'] ) === '' ):    // スマートフォンメニューを表示しない且つモバイルヘッダーに検索アイコンを表示が無効 ?>
			padding-right: 0;
			padding-left: 0;
		<?php elseif ( trim( $GLOBALS['stdata80'] ) === '' && trim( $GLOBALS['stdata479'] ) === '' ): // スマートフォンメニューを表示 + モバイルヘッダーに検索アイコンを表示が無効 ?>
			padding-right: 62px;
			padding-left: 0;
		<?php elseif ( trim( $GLOBALS['stdata80'] ) !== '' && trim( $GLOBALS['stdata479'] ) !== '' ): // スマートフォンメニューを表示しない + モバイルヘッダーに検索アイコンを表示 ?>
			padding-right: 0;
			padding-left: 62px;
		<?php endif; ?>
	}
...
<?php if ( st_is_mobile() && ( $st_mobile_logo_on || st_has_additional_mobile_menu() ) && ( $st_menu_smartbar_bg_image || $st_menu_smartbar_bg_color ) ): //　モバイル閲覧時 + 追加メニューまたはモバイルロゴ + スライドバーに背景色又は背景画像がある場合 ?>
...
<?php if ( ( ! $st_mobile_logo_on && st_has_additional_mobile_menu() ) || ! st_has_mobile_header_content() ): // モバイル用ロゴ（又はタイトル）が無効 + 追加メニューあり 又は モバイルヘッダーにコンテンツがない ?>
...
<?php elseif ( (trim( $st_h2_designsetting) !== '') && ($st_h2_designsetting === 'underdotdesign') ): ?>
	...
	.post h2:not(.st-css-no) {
		...
		<?php if ( ( trim( $st_h2_bgcolor ) !== '' ) && ( trim( $st_h2_bgcolor_t ) !== '' ) ): ?>
            ...
            <?php if( $st_h2_bgimg ): //背景画像がある場合 ?>
				.h2modoki,
                .post h2:not(.st-css-no):not(.st-matome):not(.rankh2):not(.post-card-title):not(#reply-title) {
                    background-position: <?php echo $st_h2_bgimg_side; ?> <?php echo $st_h2_bgimg_top; ?>;
                    <?php if ( $st_h2_bgimg_repeat ): ?>
                        background-repeat: no-repeat;
                    <?php endif; ?>
...
<?php if($st_h4_no_css): //カスタマイザーのCSSを無効化
	...
	<?php else: ?>
		...
		<?php if( $st_h4_bg_radius && $st_h4bordercolor && ! $st_h4_top_border && ! $st_h4_bottom_border  ): // 角丸で左ボーダーだけがある場合 ?>
			.h4modoki,
			.post h4:not(.st-css-no):not(.st-matome):not(.rankh4):not(.point) {
				border: none;
				position: relative;
...
<?php if($st_h5_no_css): //カスタマイザーのCSSを無効化
	...
	<?php else: ?>
		...
		<?php if( $st_h5_bg_radius && $st_h5bordercolor && ! $st_h5_top_border && ! $st_h5_bottom_border  ): // 角丸で左ボーダーだけがある場合 ?>
			.h5modoki,
			.post h5:not(.st-css-no):not(.st-matome):not(.rankh5):not(.post-card-title):not(#reply-title):not(.st-cardbox-t):not(.kanren-t):not(.popular-t):not(.post-card-title) {
				border: none;
				position: relative;
...
<?php // スマホヘッダー用メニュー（横列）、またはモバイル用「ヘッダー用メニュー（横列）」メニューの固定表示が有効 ?>
...
/* ヘッダー画像エリア下ウィジェット */
<?php if ( $st_header_under_bgcolor ): ?>
...
<?php if ( isset($GLOBALS['stdata375']) && $GLOBALS['stdata375'] === 'yes' ): // LPワイドの左右にシャドウ -EX ?>
	<?php if ( ( trim(  $GLOBALS['stdata366'] ) === '' ) &&  $st_h2_design_wide ): ?>
		...
		@media only screen and (max-width: 959px) {
			...
			.colum1.st-lp-wide.st-lp-wide-shadow .entry-content h2:not(.st-css-no)
			{
				margin-left: -20px;
				margin-right: -20px;
				<?php if ( ($st_h2_designsetting !== 'dotdesign') && ($st_h2_designsetting !== 'linedesign') ): //ドットデザイン及びセンターラインデザインではない場合 ?>
	...
	<?php if ( ( trim(  $GLOBALS['stdata366'] ) === '' ) &&  $st_h3_design_wide ): ?>
		...
		@media only screen and (max-width: 959px) {
			...
			.colum1.st-lp-wide.st-lp-wide-shadow .entry-content h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title)
			{
				margin-left: -20px;
				margin-right: -20px;
				<?php if ( ($st_h3_designsetting !== 'dotdesign') && ($st_h3_designsetting !== 'linedesign') ): //ドットデザイン及びセンターラインデザインではない場合 ?>
	...
	<?php else: ?>
		<?php if ( ( trim(  $GLOBALS['stdata366'] ) === '' || ! st_wrap_class_check() ) &&  $st_h3_design_wide ): ?>
			...
			<?php if ( !$st_h3_text_center && ($st_h3_designsetting === 'hukidasidesign') ): //ふきだしデザインでセンター寄せではない場合 ?>
        		...
        		.colum1 .entry-content h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title):before,
        		.colum1 .entry-content .h3modoki:after,
        		.colum1 .entry-content h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title):after
        		{
                	left:55px;
        		}
			<?php endif; ?>
	...
	<?php else: //h3 ?>
		<?php if ( ( trim(  $GLOBALS['stdata366'] ) === '' || ! st_wrap_class_check() ) &&  $st_h3_design_wide ): ?>
			...
			<?php if ( isset( $st_h3_designsetting ) && ( $st_h3_designsetting === 'hukidasidesign_under') ): //吹き出し下線デザインの場合 ?>
				...
				.colum1 .post h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title):before {
					left: 90px;
				}
				.colum1 .h3modoki:after,
				.colum1 .post h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title):after {
					left: 93px;
				}
			<?php endif; ?>
			...
			<?php if ( isset( $st_h3_designsetting ) && ( $st_h3_designsetting === 'checkboxdesign' || $st_h3_designsetting === 'linedesign') &&  ! $st_h3_bgimg_leftpadding ): //左余白が空でチェックボックス又は左囲みデザインの場合 ?>
				...
				.colum1 .h3modoki,
				.colum1 .post h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title) {
					<?php if ( $st_h3_designsetting === 'checkboxdesign' ): ?>
						padding-left: calc(1.5em + 90px)!important;
					<?php else: ?>
						padding-left: 100px!important;
			...
			<?php if ( isset( $st_h3_designsetting ) && ( $st_h3_designsetting === 'dotdesign') &&  ! $st_h3_bgimg_leftpadding ): //左余白が空で囲みドットデザインの場合 ?>
				.h3modoki span.st-dash-design,
				.post h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title) span.st-dash-design {
					<?php if ( $st_area ): //記事幅が広い場合 ?>
						padding-left:20px;
						padding-right:20px;
					<?php else: ?>
				...
				.colum1 .post h3:not(.st-css-no):not(.st-matome):not(.rankh3):not(.post-card-title):not(#reply-title) span.st-dash-design {
					padding-left: 70px!important;
					padding-right: 70px!important;
				}
			<?php endif; ?>
...
<?php if ( ! st_has_header_content() ): // ヘッダーエリアのコンテンツが空 ?>
	...
	<?php if ( ! st_mobilelogo_on() ): // 'スマホ・タブレット表示時にモバイル用ロゴ及びタイトルの使用'が無効 ?>
		@media print, screen and (max-width: 959px) {
			#st-text-logo {
			    padding-bottom: 0;
			}
